...
class DataIngestion:
    def __init__(self, config, gcp_json_path=None):
        ...
        if gcp_json_path:
            ...
            logger.info(f"GCP credentials set from {gcp_json_path}")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python -m src.data_ingestion <GCP_JSON_FILE_PATH>")
        sys.exit(1)

    gcp_json_path = sys.argv[1]
    config = read_yaml(CONFIG_PATH)
    data_ingestion = DataIngestion(config, gcp_json_path)
    data_ingestion.run()
